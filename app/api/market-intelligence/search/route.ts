...
// 1. Paksa Next.js agar selalu mengeksekusi rute ini secara LIVE (Bypass Vercel Edge Cache)
...
// 2. PASTIKAN keyword 'export' dan 'async function GET' ditulis persis seperti ini!
  ...
  try {
    ...
    if (!accessToken || !appKey || !appSecret) {
      return NextResponse.json({ 
        ...
        message: "Periksa environment variables di dashboard Vercel Anda."
      }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: "Respon dari TikTok bukan JSON" }, { status: 400 });
    }

    if (!tiktokResponse.ok || (tiktokData.code !== 0 && tiktokData.code !== 200)) {
      return NextResponse.json({
        error: `TikTok API Error (${tiktokData.code})`,
        message: tiktokData.message || "Akses ditolak oleh API TikTok."
      }, { status: 400 });
    }
    ...
    if (!dataObj.products || dataObj.products.length === 0) {
      return NextResponse.json({
        ...
        products: [],
        message: "Pencarian sukses, tidak ada produk ditemukan."
      });
    }
