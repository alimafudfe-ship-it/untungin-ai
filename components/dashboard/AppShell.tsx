...
const navGroups: { label: string; items: { key: TabKey; label: string; helper: string; icon: string }[] }[] = [
  { label: "Operasional", items: [
    { key: "overview", label: "Dashboard", helper: "Command center", icon: "🏠" },
...
const meta: Record<TabKey, { title: string; subtitle: string }> = {
  overview: { title: "Dashboard Operasional", subtitle: "Pantau profit, stok, cashflow, risiko, dan action plan harian" },
  ...
  pricing: { title: "Subscription & Billing", subtitle: "Upgrade PRO, Xendit, manual transfer, dan fallback pembayaran" },
};
...
export function AppShell({ activeTab, onTabChange, isPro, proExpired, onExport, onUpgrade, onLogout, children }: { activeTab: TabKey; onTabChange: (tab: TabKey) => void; isPro: boolean; proExpired: boolean; onExport: () => void; onUpgrade: () => void; onLogout: () => void; children: React.ReactNode }) {
  ...
  `}</style><div className="app-shell" style={{ display: "grid", gridTemplateColumns: "280px 1fr", minHeight: "100vh" }}>
    <aside className="sidebar" style={{ position: "sticky", top: 0, height: "100vh", padding: 16, background: "rgba(255,255,255,0.82)", borderRight: "1px solid rgba(16,24,40,0.09)", backdropFilter: "blur(18px)", display: "flex", flexDirection: "column", justifyContent: "space-between", gap: 14 }}>
      <div className="sidebar-scroll" style={{ overflowY: "auto", paddingRight: 4 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "8px 8px 22px" }}><div style={{ width: 46, height: 46, borderRadius: 16, display: "grid", placeItems: "center", background: "linear-gradient(135deg,#0f766e,#14b8a6)", color: "white", fontWeight: 950, boxShadow: "0 16px 34px rgba(15,118,110,0.24)" }}>U</div><div><strong style={{ fontSize: 18 }}>Untungin.ai</strong><div style={{ color: "#667085", fontSize: 12 }}>Seller Command Center</div></div></div>
        <nav style={{ display: "grid", gap: 12 }}>{navGroups.map((group) => <div key={group.label} style={{ display: "grid", gap: 6 }}><div style={{ padding: "0 10px", color: "#98a2b3", fontSize: 11, fontWeight: 900, letterSpacing: 0.7, textTransform: "uppercase" }}>{group.label}</div>{group.items.map((item) => <button key={item.key} onClick={() => onTabChange(item.key)} style={{ display: "grid", gridTemplateColumns: "34px 1fr", alignItems: "center", gap: 9, textAlign: "left", border: activeTab === item.key ? "1px solid rgba(15,118,110,0.18)" : "1px solid transparent", borderRadius: 16, padding: "9px 10px", background: activeTab === item.key ? "linear-gradient(135deg,#0f766e,#14b8a6)" : "transparent", color: activeTab === item.key ? "white" : "#475467", cursor: "pointer", fontWeight: 850, boxShadow: activeTab === item.key ? "0 14px 30px rgba(15,118,110,0.20)" : "none" }}><span style={{ width: 30, height: 30, borderRadius: 12, display: "grid", placeItems: "center", background: activeTab === item.key ? "rgba(255,255,255,0.14)" : "#f2f4f7" }}>{item.icon}</span><span>{item.label}<br /><small style={{ color: activeTab === item.key ? "#dcfffb" : "#667085", fontWeight: 650 }}>{item.helper}</small></span></button>)}</div>)}</nav>
      </div>
      <div style={{ padding: 15, borderRadius: 20, background: "linear-gradient(180deg,#ffffff,#f8fafc)", border: "1px solid rgba(16,24,40,0.10)", boxShadow: "0 18px 45px rgba(16,24,40,0.07)" }}><small style={{ color: "#667085" }}>Status akun</small><br /><strong>{isPro ? "PRO aktif" : proExpired ? "PRO expired" : "Free plan"}</strong><p style={{ margin: "8px 0 0", color: "#667085", fontSize: 12, lineHeight: 1.55 }}>AI CFO, report otomatis, dan workspace tim.</p><button onClick={onUpgrade} style={{ ...ctaButtonStyle, width: "100%", marginTop: 12 }}>Upgrade PRO</button></div>
    </aside>
    <section className="content" style={{ padding: 22, maxWidth: 1600, width: "100%" }}>
      <div className="topbar" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 14, flexWrap: "wrap", background: "rgba(255,255,255,0.84)", border: "1px solid rgba(16,24,40,0.09)", borderRadius: 24, padding: "13px 16px", marginBottom: 18, backdropFilter: "blur(16px)", boxShadow: "0 18px 55px rgba(16,24,40,0.06)" }}><div><strong style={{ fontSize: 18 }}>{current.title}</strong><div style={{ color: "#667085", fontSize: 12, marginTop: 2 }}>{current.subtitle} · {new Date().toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric" })}</div></div><div className="nav-actions" style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", justifyContent: "flex-end" }}><Badge label={isPro ? "PRO" : proExpired ? "Expired" : "Free"} tone={isPro ? "success" : proExpired ? "danger" : "warning"} /><button onClick={() => setDarkMode((value) => !value)} style={ghostButtonStyle}>{darkMode ? "Mode terang" : "Mode gelap"}</button><button onClick={onExport} style={ghostButtonStyle}>Export data</button>{!isPro && <button onClick={onUpgrade} style={ctaButtonStyle}>Upgrade</button>}<button onClick={onLogout} style={{ ...ghostButtonStyle, color: "#b42318", background: "#fff7f7", borderColor: "#fecaca" }}>Logout</button></div></div>{children}</section>
